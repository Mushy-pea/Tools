...
load_map (x:xs) w u v u_limit v_limit w_grid =
  ...
  else if v == v_limit then load_map xs w (u + 1) 0 u_limit v_limit (w_grid // [((w, u, v), x)])
  else load_map xs w u (v + 1) u_limit v_limit (w_grid // [((w, u, v), x)])
...
save_map w u v w_limit u_limit v_limit acc w_grid =
  ...
  else if v == v_limit then save_map w (u + 1) 0 w_limit u_limit v_limit (acc ++ " " ++ w_grid ! (w, u, v) ++ "\n") w_grid
  else save_map w u (v + 1) w_limit u_limit v_limit (acc ++ " " ++ w_grid ! (w, u, v)) w_grid
...
add_map_border w u v u_limit v_limit w_grid =
  if w > 2 then w_grid
  else if u > u_limit && v > v_limit then add_map_border (w + 1) 0 0 u_limit v_limit w_grid
  else if v > v_limit then add_map_border w (u + 1) 0 u_limit v_limit w_grid
  else if v == 0 then add_map_border w u (v + 1) u_limit v_limit (w_grid // [((w, u, v), 'c' : tail (w_grid ! (w, u, v)))])
  else if v == v_limit then add_map_border w u (v + 1) u_limit v_limit (w_grid // [((w, u, v), 'b' : tail (w_grid ! (w, u, v)))])
  else if u == 0 then add_map_border w u (v + 1) u_limit v_limit (w_grid // [((w, u, v), 'i' : tail (w_grid ! (w, u, v)))])
  else if u == u_limit then add_map_border w u (v + 1) u_limit v_limit (w_grid // [((w, u, v), 'e' : tail (w_grid ! (w, u, v)))])
  else add_map_border w u (v + 1) u_limit v_limit w_grid
